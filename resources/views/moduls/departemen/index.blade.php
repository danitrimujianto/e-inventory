    ...
    <div class="box">
      <div class="box-header">
        ...
        <form method="get" action="{{ '/'.$theme['modul'] }}">
              ...
              <option value="code" @if($sf == "code") {{ 'selected' }} @endif>Departemen Code</option>
              <option value="name" @if($sf == "name") {{ 'selected' }} @endif>Departemen Name</option>
  					</select>
					</div>
				  <div class="col-md-5 col-xs-12">
      ...
      <div class="box-body table-responsive no-padding">
        <table class="table table-hover">
          ...
          @foreach($data AS $d)
          <tr data-id="{{ $d->id }}" data-field="{{ 'Departemen' }}" data-value="{{ $d->name }}">
            <td>{{ $d->code }}</td>
            <td>{{ $d->name }}</td>
            <td>{{ $d->remarks }}</td>
            <td>
